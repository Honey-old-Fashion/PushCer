...
namespace VeryAnimation
{
    public class UAnimationWindow //560
    {
        ...
        {
            if (instance == null) return null;
            return uAnimationWindowSelectionItem.GetGameObject(selectedItem);
        }
        public void SetSelectionGameObject(GameObject gameObject, string undoName = null)
        {
            if (instance == null) return;
            if (GetSelectionGameObject() == gameObject) return;
            var si = selectedItem;
            if (si == null) return;
            if (undoName != null)
                Undo.RecordObject(selectedItem as UnityEngine.Object, undoName);
            uAnimationWindowSelectionItem.SetGameObject(selectedItem, gameObject);
            ResampleAnimation();
            Repaint();
        }
